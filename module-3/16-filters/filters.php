...
foreach ($_GET as $filter => $values) {
    // if any values are not array's the [] do that 
...
?>
    <div class="btn-group mb-3" role="group" aria-label="<?= $filter ?>Filter Group">
        ...
        <?php foreach ($options as $key => $value) : ?>
            <?php 
                $is_active = in_array($key, $active_filters[$filter] ?? []);
                $url = build_query_url($_SERVER['PHP_SELF'], $active_filters, $filter,$key);    
            ?>

            <a href="<?= $url ?>" class="btn 
            <?php echo ($is_active) ? 'btn-success' : 'btn-outline-success' ; ?>
            " aria-pressed="<?php ($is_active) ? 'true' : 'false'; ?>"><?= $value ?></a>
...
if (!empty($active_filters)) {
    echo '<div class="my-5"><a href="filters.php" class="btn btn-danger">Clear Filters</a></div>'; include 'includes/filter-results.php';
}
?>
